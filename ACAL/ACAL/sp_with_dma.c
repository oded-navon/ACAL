...
static void sp_ctl(sp_t *sp)
{
	...
	//printf("spro->ctl_state is: %d, and dma is: %d, dma_received is: %d\n", spro->ctl_state, ctl_dma_state, dma_opcode_received);
	...
	case CTL_STATE_DEC1:
		...
		//	perform_dma_logic(mem_available, sp);
		//}
		if (spro->opcode == DMA && !dma_opcode_received) //in case DMA is already working, we ignore the new request
		{
	...
	case CTL_STATE_EXEC1:
		...
		//printf("spro->ctl_state in exec1: %d, and dma_opcode: %d\n", spro->ctl_state, dma_opcode_received);
		//if (spro->ctl_state == CTL_STATE_IDLE && !dma_opcode_received) 
		//{
...
POL command:
POL Ra _ _ - saves into Ra whether the DMA is available (1) or not (0)
*/
...
void perform_dma_logic(bool mem_available, sp_t* sp)
{
	...
	case(NO_READ_WRITE):
		...
		if (dma_regs[2] == 0)
		{
			llsim_printf("dma finished his job\n");
			dma_opcode_received = false;
			ctl_dma_state = DMA_IDLE_STATE;
		}
